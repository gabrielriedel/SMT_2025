...
export default function GifDisplay() {
  ...
  const handleSubmit = () => {
    setLoading(true);
    fetch("https://smt-2025.onrender.com/api/run_animation").then((res) => {
  ...
  return (
    <main className="flex flex-col px-6 py-10 max-w-6xl mx-auto gap-8">
      <h1 className="text-3xl font-bold text-center text-green-800">
        Random Run Scored Play Generator
      </h1>
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <section className="bg-white border-l-4 border-green-600 shadow-md p-6 rounded-lg w-full">
            <p className="text-gray-700 text-lg">
              The main model of the scouting report incorporates the run differential of the game at the time 
              of a play. In order to do that we have to find plays where runs are scored. This visualization 
          ...
          <div className="bg-green-100 border border-green-400 p-4 rounded-lg shadow-inner w-full max-w-xl text-center">
            {loading ? (
              ...
              <p className="text-green-800">
                Click the button to generate an animation of a play where a run was scored.
              </p>
            )}
          </div>
        </div>
