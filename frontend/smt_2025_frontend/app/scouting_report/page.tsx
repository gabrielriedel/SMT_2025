...
interface Props {
  onSelectionChange: (team: string, pitcher: string) => void;
}

export default function PitcherScoutingReport({ onSelectionChange }: Props) {
  ...
  useEffect(() => {
    if (selectedTeam && selectedPitcher) {
      onSelectionChange(selectedTeam, selectedPitcher);
    }
  }, [selectedPitcher]);
